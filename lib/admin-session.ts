...
export function getAdminEmail() {
  return (process.env.ADMIN_PANEL_EMAIL || 'admin@92glory786.com').trim();
}

export function getAdminPassword() {
  return (process.env.ADMIN_PANEL_PASSWORD || 'admin@786').trim();
}
...
export function verifyAdminSession(token: string | undefined | null) {
  ...
  if (payload.email?.trim().toLowerCase() !== getAdminEmail().toLowerCase()) return null;
  return payload;
}
