...
export default function WithdrawHistoryPage() {
  ...
  const [transactions, setTransactions] = useState<WithdrawRow[]>([]);
      ...
      <div className="p-4 space-y-3">
        {transactions.map((trx) => (
          <div key={trx.id} className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
            ...
            <p className="text-[11px] text-gray-500">{String(trx.meta?.accountTitle || trx.meta?.method || 'WITHDRAW')}</p>
        ...
        {!transactions.length && (
          <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
            <p className="text-xs text-gray-500">No withdrawal history found.</p>
          </div>
        )}
      </div>
    </div>
